...
<section class="section-fluid mt-100">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <h1 class="hps_heading"><?php echo $TagName; ?></h1>
                <h2 class="hps_sub_heading">THE BEST ARTICLES</h2>
            </div>
        </div>
        <div class="row best_post">
            <?php  
                $blog_tag_recent_post_arg = array(
                    'post_type' => 'post',
                    'tag_id' => $Tag_ID,
                    'posts_per_page' => 1,
                    //'order'   => 'DESC',
                ...
                $blog_tag_recent_post = new WP_Query( $blog_tag_recent_post_arg ); 
                $blog_tag_recent_post_id = '';

                if ( $blog_tag_recent_post->have_posts() ) {
                    while ( $blog_tag_recent_post->have_posts() ) {
                        $blog_tag_recent_post->the_post();
                        // do stuff
                        $blog_tag_recent_post_id = get_the_id();
                ...
                <div class="col-md-6">
                    ...
                    <?php
                    $blog_tag_recent_post_categories = wp_get_post_categories( $blog_tag_recent_post_id );
                    $blog_tag_recent_post_cats = array();
                         
                    foreach($blog_tag_recent_post_categories as $c){
                        $cat = get_category( $c );
                        $blog_tag_recent_post_cats[] = array( 'name' => $cat->name, 'slug' => $cat->slug );
                    }
                    /*var_dump( $blog_tag_recent_post_cats);*/
                    ?>
                        <ul class="best_post_details_categories">
                            <?php foreach($blog_tag_recent_post_cats as $blog_tag_recent_post_cat){?>  
                                <li><a href="<?php echo $blog_tag_recent_post_cat['slug'];?>"><?php echo $blog_tag_recent_post_cat['name'];?></a></li>
                            <?php }?>  
                        </ul>
                        ...
                        <?php 
                          $voices = get_post_meta($blog_tag_recent_post_id, "voices");
                          $voices = ( !empty($voices[0]) ) ? $voices[0] : 0;
                          $rating = get_post_meta($blog_tag_recent_post_id, "rating");
...
<section class="section-fluid">
    <div class="container-fluid">
        ...
        $BlogCategoryPosts_arg = array(
            ...
            'posts_per_page' => 9,
            'order'   => 'DESC',
            'post__not_in' => array($blog_tag_recent_post_id),
            'paged'     => $paged, 
